...
export const drawHistory = (log: Record<string, number[]>) => {
    ...
    const getYPosition = (y: number) =>
        canvas.height - axisValueOffset - y * yStep;
    ...
    logValues[0].forEach((_, index) => {
        ...
        drawText(
            String(index),
            [getXPosition(index), canvas.height - axisValueOffset / 2],
            9,
            '#000',
        );
    });
    ...
    Array.from({ length: endMaxYLineValue }).forEach((_, index) => {
        ...
        const strYValue = isExponentialValuesPrint
            ? yValue.toExponential()
            : String(yValue);
    ...
    Object.values(log).forEach((data, index) => {
        ...
        data.map((value, epoch) => {
            epoch === 0
                ? context.moveTo(
                      getXPosition(epoch),
                      getYPosition(value * endYMultiplier),
                  )
                : context.lineTo(
                      getXPosition(epoch),
                      getYPosition(value * endYMultiplier),
    ...
    drawRect(
        [2 + axisValueOffset, 0],
        [100, (logNames.length + 1) * lineHeight],
        backgroundColor,
    );
    logNames.forEach((name, index) => {
        ...
        drawCircle(
            [7 + axisValueOffset, (index + 1) * lineHeight - radius / 2],
            radius,
            color,
        );
        drawText(
            name,
            [9 + radius * 2 + axisValueOffset, (index + 1) * lineHeight],
            fontSize,
            color,
            'left',
        );
